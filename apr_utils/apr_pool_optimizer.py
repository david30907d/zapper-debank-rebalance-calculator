    ...
    for portfolio in categorized_positions.values():
        for symbol in portfolio["portfolio"].keys():
            ...
            for pool_metadata in res_json['data']:
                if search_handler(symbol, pool_metadata['symbol']) and get_metadata_by_symbol(symbol).get('defillama-APY-pool-id', None) != pool_metadata['pool'] and current_apr < convert_apy_to_apr(pool_metadata['apyMean30d']/100):
...
def _get_search_handler(searching_algorithm):
    if searching_algorithm == 'ngram':
        threashold = 0.2
        return lambda symbol, compared_symbol: ngram.NGram.compare(symbol.lower(), compared_symbol.lower()) > threashold
    else:
        raise NotImplementedError(f"search algorithm {searching_algorithm} not implemented")

def _print_out_topn_candidate_pool(symbol: str, top_n: list, current_apr: float, n: int=3):
    if not top_n:
        return
    print(f"{symbol}'s possible better protocol to deposit (Current apyMean30d {current_apr:.2f}):")
    for metadata in sorted(top_n, key=lambda x: -x['apyMean30d'])[:n]:
        print(f" - Chain: {metadata['chain']}, Protocol: {metadata['project']}, Token: {metadata['symbol']}, APR: {((metadata['apyMean30d']/100+1)**(1/365)-1)*365:.2f}")