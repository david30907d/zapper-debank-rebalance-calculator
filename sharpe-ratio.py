...
def calculate_max_drawdown(positions: dict):
    ...
    return (net_worth['net_worth'].max() - net_worth['net_worth'].min()) / net_worth['net_worth'].max()

def _get_daily_returns(positions: dict):
    df = pd.DataFrame()
    for symbol, tokenBalance in positions.items():
        price_pd = _get_token_historical_price(symbol)
        daily_returns_per_token = _calculate_daily_returns_per_token(price_pd, tokenBalance)
